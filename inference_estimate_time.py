...
parser.add_argument('--mode', type=str, choices=['detect', 'synthesize'], required=True,
                    help='Mode of operation: "detect" to run face detection and save results, '
                         '"synthesize" to generate the lip-synced video.')
...
def log_time(func):
    """Decorator to calculate the time taken by functions"""
...
@log_time
def process_face_detection(face_path):
    # Read video or image
    ...
    print("Number of frames available for face detection: " + str(len(full_frames)))
    ...
    print('Face detection complete and results saved.')
...
def main():
    if args.mode == 'detect':
        process_face_detection(args.face)
        return
    ...
    if not os.path.exists('temp/face_det_results.pkl'):
        raise FileNotFoundError('Face detection results not found. Please run the script with --mode detect first.')
    else:
        print('Loading face detection results...')
        face_det_results = load_face_det_results()

    if not args.audio:
        raise ValueError('Please provide the audio file path using --audio argument.')
    ...
    # Start timing for audio processing
    start_time = time.time()
    wav = audio.load_wav(args.audio, 16000)
    mel = audio.melspectrogram(wav)
    print(mel.shape)
    end_time = time.time()
    print(f"Audio processing took {end_time - start_time:.4f} seconds.")
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen,
        ...
        end_time = time.time()
        print(f"Model inference for batch {i} took {end_time - start_time:.4f} seconds.")
